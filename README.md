...
在用input获取用户输入的字符串时，直接在终端输入字符串报**is not defined.

查了资料才知道input() ，它希望能够读取一个合法的 python 表达式，即你输入字符串的时候必须使用引号将它括起来，否则它会引发一个 SyntaxError 。

如果感觉输入字符串还要在终端输入引号好麻烦，我们可以用raw_input(),raw_input()可以直接读取控制台的输入，raw_input() 将所有输入作为字符串看待，返回字符串类型。而 input() 在对待纯数字输入时具有自己的特性，它返回所输入的数字的类型（ int, float ）
### 6.8常量与pylint的规范
- 每个文件开头都要写功能简介
- if后面的不要有空格
- 常量要大写
...
//python2.7中input()获取到的用户输入的数字是整型，python3中input()获取到用户输入的数字是字符串型,需要转换
a = int(a);
